...
def transcribe_video(video_location: pl.Path,
    ...
    Args:
        ...
        chunk_duration (int):           size of the audio chunks in seconds. The function will split the input video's audio
                                        ...
                                        seconds etc. These chunks will be send to the openai API for transcription. This is
                                        necessary since the API cannot deal with files that are larger than 25MB. Therefore,
                                        you must make sure that the resulting parts do not exceed this size. The default of
                                        900 seconds equals 15 minutes and most often ensures that the resulting chunks are
                                        below 25 MB.
        output_location (Path | None):  output file to save the transcription in. If None, transcription will be saved
                                        in the same directory as the input video under the same name in the .txt format.
    '''
    # creating file and directory names
    audio_file_location = create_file_in_same_dir(input_file=video_location, file_ending='_audio.mp3')
    # audio_file_location = video_location.parent
    # file_name = video_location.parts[-1]
    # file_name = file_name[0:file_name.rfind('.')] + '_audio.mp3'
    # audio_file_location = audio_file_location / file_name
    file_name = video_location.parts[-1]
    chunk_file_folder = audio_file_location.parent / file_name[0:file_name.rfind('.')]
    chunk_file_folder.mkdir()

    if output_location is None:
        output_location = create_file_in_same_dir(input_file=video_location, file_ending='_transcript.txt')

    extract_audio(video=video_location, output_location=audio_file_location)
    create_audio_chunks(audio = audio_file_location, chunk_duration = chunk_duration, output_location = chunk_file_folder)
    ...
    for audio_chunk_number, audio_chunk in enumerate(list(chunk_file_folder.glob('**/*.mp3'))):
        ...
        chunk_transcription = transcribe_audio(audio=audio_chunk)
